...
def VFPoint(x,y,w,h): # Flip point Horizontally
    if x<=w and y<=h:
        return w-x, y
    else: 
        print("x, y should be smaller than w, h")
        return -1

def HFPoint(x,y,w,h): # Flip point Vertically
    if x<=w and y<=h:
        return x, h-y
    else: 
        print("x, y should be smaller than w, h")
...
def flip_with_point(im, x, y, axis, w=1, h=1):
    """Flip with a single point
    axis: str
          z - flip along Depth  (z-axis)
          y - flip along Height (y-axis)
          x - flip along Width  (x-axis)
    ...
    if axis == 'x':
        x, y = HFPoint(x,y,w,h)    # w,h are not defined 
    elif axis == 'y':
        x, y = VFPoint(x,y,w,h)
    else:
...
def flip_with_points(im, x, y, axis,confidence, w=1, h=1):
    """Flip with multiple points
    axis: str
          z - flip along Depth  (z-axis)
          y - flip along Height (y-axis)
          x - flip along Width  (x-axis)
    ...
    if axis == 'x':
        for i in range(len(x)):
            if confidence[i] >= 0.5:
                x[i], y[i] = HFPoint(x[i],y[i],w,h)    # w,h are not defined 
            # else (confidence < 0.5) remain original value (0,0)
    elif axis == 'y':
        for i in range(len(y)):
            if confidence[i] >= 0.5:
                x[i], y[i] = VFPoint(x[i],y[i],w,h)
            # else (confidence < 0.5) remain original value
    else:
        print("axis should be either x or y")
...
def rotate_with_points(img, angle, x, y, confidence, resample=False, expand=False, center=None, centerx=0.5, centery=0.5):
    ...
    for i in range(len(x)):
        ...
        # else just keep the orginal coordinates
    if img.shape[2] == 1:
        return cv2.warpAffine(img, M, (cols, rows))[:, :, np.newaxis], x,y
    else:
        return cv2.warpAffine(img, M, (cols, rows)), x,y
