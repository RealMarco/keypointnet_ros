...
def PCinfer(model1,model2, test_filelists2, batchsize=8):  # model3,
    '''
...
def KPinfer(model, test_filelists2, batchsize=8, state_mode = False, orient_mode = False): # state_mode and orient_mode are post processing mode flag
    '''
...
#            fws = data[5].astype('int8')
            ...
            for i in range(logits.shape[0]):  # batchsize
                ...
                end4 = time.time()
                end4 = time.time()
                processing_time4.append(end4 - start4)
                ...
                end3 = time.time()
                processing_time3.append(end3 - start3)
    
    avg_time4 = sum(processing_time4)/len(processing_time4)
    avg_time3 = sum(processing_time3)/len(processing_time3)
    #print('len(processing_time3): %f'%(len(processing_time3)))
    print("Average inference time for keypoint-based state classification   : %f"%(avg_time3))
    print("Average inference time for keypoint-based orientation estimation : %f"%(avg_time4))
